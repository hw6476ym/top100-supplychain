...
let companies=[],materialLedger=[],scenarios=[];
const materials=["Copper","Silicon wafers","HBM memory","Lithium","Nickel","Graphite","Rare earth magnets","Power semiconductors"];
...
async function loadData(){
  [companies,materialLedger,scenarios]=await Promise.all([
...
function initControls(){
  ...
  byId('materialSelect').innerHTML=materials.map(m=>`<option>${m}</option>`).join('');
...
function renderCards(){
  byId('companyCards').innerHTML=companies.map(c=>`<div class='card' data-name='${c.name}'><h3>${c.name} <span class='risk-${c.resilienceScore>69?'low':'high'}'>${c.resilienceScore}</span></h3><div class='kpi'><span>Overall risk</span><b>${avg(c.riskScores)}</b></div><div class='kpi'><span>Semi exposure</span><b>${c.riskScores[0]}</b></div><div class='kpi'><span>Energy sensitivity</span><b>${c.riskScores[10]}</b></div></div>`).join('');
...
function updateCompany(name){
  ...
  byId('companyProfile').innerHTML=`<b>${c.name}</b> (${c.ticker})<br><b>Segments:</b> ${c.segments.join(', ')}<br><b>Chain narrative:</b> ${c.chain}`;
  ...
  Plotly.newPlot('sankeyChart',[{type:'sankey',node:{label:[c.name,'Segments','Systems','Suppliers','Materials'],color:'#23d5ff'},link:{source:[0,1,2,3],target:[1,2,3,4],value:[10,9,8,7]}}],layout('Top-to-bottom chain map'));
}
function updateMaterial(material){
  const rows=materialLedger.filter(r=>r.material===material);
  const exposed=[...new Set(rows.map(r=>r.company))];
  byId('materialInfo').innerHTML=`<b>${material}</b><br><b>Exposed companies:</b> ${exposed.join(', ')||'No records'}<br>Source quality: simulated planning records with confidence scores.`;
...
function renderShared(rows,material){
  ...
  byId('sharedLinkageTable').innerHTML='<tr><th>Shared cluster</th><th>Companies</th><th>Total tons</th><th>Weighted $/ton</th></tr>'+clusters.map(c=>`<tr><td>${c.key}</td><td>${c.companies.join(', ')}</td><td>${c.volume.toLocaleString()}</td><td>${c.weightedCost.toLocaleString()}</td></tr>`).join('');
  byId('linkageInsight').innerHTML=clusters.length?`<b>${material}:</b> ${clusters.length} shared dependency clusters found.`:`No shared clusters for ${material}.`;
}
function renderRanking(){Plotly.newPlot('rankingChart',[{type:'bar',orientation:'h',y:companies.map(c=>c.name),x:companies.map(c=>c.riskScores[0]+c.riskScores[10]-c.resilienceScore),marker:{color:'#23d5ff'}}],layout('Risk concentration ranking'));}
...
function updateScenario(name){const s=scenarios.find(x=>x.name===name);const impacts=companies.map(c=>({name:c.name,val:c.riskScores[0]}));Plotly.newPlot('scenarioImpact',[{type:'bar',x:impacts.map(i=>i.name),y:impacts.map(i=>i.val),marker:{color:'#ff5b6e'}}],layout('Scenario severity view'));byId('scenarioExplain').innerHTML=`<b>${s.name}</b><br><b>Channel:</b> ${s.channel}<br><b>Relative beneficiaries:</b> ${s.beneficiaries}`;}
function renderStrategic(){byId('strategicList').innerHTML=["Shared supplier-route clusters indicate concentrated systemic risk.","AI infrastructure is physically constrained by memory, power, copper, and packaging.","Use confidence + as_of_date fields before treating any value as decision-grade."].map(x=>`<li>${x}</li>`).join('');}
function layout(title){return {title,font:{color:'#d7e4ff'},paper_bgcolor:'#0f1728',plot_bgcolor:'#0f1728',margin:{l:45,r:20,t:45,b:60}};}
window.addEventListener('load',async()=>{await loadData();initControls();renderCards();renderRanking();renderHeatmap();renderStrategic();updateCompany(companies[0].name);updateMaterial('Copper');updateScenario(scenarios[0].name);byId('companySelect').addEventListener('change',e=>updateCompany(e.target.value));byId('materialSelect').addEventListener('change',e=>updateMaterial(e.target.value));byId('scenarioSelect').addEventListener('change',e=>updateScenario(e.target.value));});
